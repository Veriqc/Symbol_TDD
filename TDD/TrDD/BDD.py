...
class BDD:
    ...
    def __eq__(self, other):
        return (
            ...
            and math.isclose(self.weight.real, other.weight.real, rel_tol=epi)
            and math.isclose(self.weight.imag, other.weight.imag, rel_tol=epi)
        )
            ...
            for succ2 in tdd2.node.successor:
                ...
                if not succ1[0] + succ2[0] == 0:
                    if succ1[0] + succ2[0] == 2 and k1[:3] == "sin":
                        temp_res1 = normalize(
                            "cos" + k1[3:], [[2, -1, Find_Or_Add_Unique_table(-1)]]
                        )
...
def add(tdd1, tdd2):
    ...
    if k1 == k2:
        ...
        the_successor += [
            [succ[0], succ[1] * tdd1.weight, succ[2]]
            for succ in tdd1.node.successor
            if not succ[0] in degree2
        ]
        the_successor += [
            [succ[0], succ[1] * tdd2.weight, succ[2]]
            for succ in tdd2.node.successor
        ...
        if tdd1.node.successor[0][0] == 0:
            ...
            the_successor += [
                [succ[0], succ[1] * tdd1.weight, succ[2]]
                for succ in tdd1.node.successor[1:]
            ]
        else:
        ...
        if tdd2.node.successor[0][0] == 0:
            ...
            the_successor += [
                [succ[0], succ[1] * tdd2.weight, succ[2]]
                for succ in tdd2.node.successor[1:]
            ]
        else:
    ...
    for key in tdd1.node.index_degree:
        if key in tdd2.node.index_degree:
            if min(tdd1.node.index_degree[key], tdd2.node.index_degree[key]) > 0:
                min_degree[key] = min(
                    tdd1.node.index_degree[key], tdd2.node.index_degree[key]
                )
...
def get_expr(node):
    ...
    for succ in node.successor:
        res += nsimplify(
            succ[1] * triangle_function ** succ[0], tolerance=1e-3
        ) * get_expr(succ[2])
        # print('BDD 757 ',res)
