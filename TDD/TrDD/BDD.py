...
class BDD:
    ...
    def show(self, real_label=True):
        ...
        dot.edge(
            ...
            str(self.node.idx),
            color="blue",
            label=str(nsimplify(self.weight, rational=False, tolerance=1e-3)),
        )
        dot.format = "png"
...
def layout(node, dot=Digraph(), succ=[]):
    ...
    dot.node(
        str(node.idx), str(node.key), fontname="helvetica", shape="circle", color="red"
    )
...
def Find_Or_Add_Unique_table(local_key, the_successors=[]):
    """To return a node if it already exist, creates a new node otherwise"""
    global global_node_idx, unique_table
    if not isinstance(local_key, int):
        print("BDD 284", local_key)
    if local_key == -1:
        if unique_table.__contains__(local_key):
            return unique_table[local_key]
        else:
            res = Node(local_key)
            res.idx = 0
            unique_table[local_key] = res
        return res
    temp_key = [local_key]
    ...
    if temp_key in unique_table:
        ...
        res.index_degree = {local_key: the_successors[0][0]}
...
def find_computed_table(item):
    ...
    if item[0] == "s":
        ...
        the_key = (
            ...
            get_int_key(item[1].weight),
            item[1].node,
            (tuple(item[2].keys()), tuple(item[2].values())),
        )
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            bdd = BDD(res[1])
...
def get_bdd(function):
    global global_index_order
    if (
        isinstance(function, int)
        or isinstance(function, float)
        or isinstance(function, complex)
    ):
        bdd = get_one_state()
        bdd.weight = function
        return bdd
...
def normalize_2_fun(bdd1, bdd2, key_2_new_key):
    # normalize_2_fun is need to normalize bdd1 and bdd2,the result is like [bdd1,1,bdd2/bdd1]
    global inverse_global_index_order
    
    if bdd1.weight == 0:
        return [bdd2.self_copy(), bdd1.self_copy(), get_one_state()]
    ...
    print("BDD 717 min_degree", min_degree_dict)
    # if min_degree_dict not empty dict() print it
    if min_degree_dict != dict():
        print("BDD 720 min_degree_dict", min_degree_dict)

    # to hash dict()
    hash1 = tuple([(k, v) for k, v in min_degree_dict_local1.items()])
    hash2 = tuple([(k, v) for k, v in min_degree_dict_local2.items()])
    
    res = find_computed_table(["/", bdd1, bdd2, (hash1, hash2)])
    
    if res:
        print('BDD 731 hit / cache')
        a, b, c = res
...
def var_sort(var):
    global global_index_order, inverse_global_index_order
    # print('BDD 849', var, inverse_g3lobal_index_order, global_index_order)
...
def mul2(bdd1, bdd2, key_2_new_key):
    ...
    if k1 == -1:
        ...
        return BDD(bdd2.node, weight=weig)

    if k2 == -1:
        if w2 == 0:
            return BDD(bdd2.node, weight=0)
        return BDD(bdd1.node, weight=weig)
...
# following preserved for further use?
...
def hashed_args(func):
    ...
    update_wrapper(
        wrapper, func, ("cache_info", "cache_clear", "cache_parameters")
    )  # 繼承lru_cache的屬性
    return wrapper
