...
def normalize(x,the_successors):
    ...
    the_first_num_zero_element=0
    
    for k in range(len(the_successors)):
        if weigs[k]!=0:
            the_first_num_zero_element=k
            break
    the_pivot=weigs[the_first_num_zero_element]
#     print(weigs,the_pivot)
    weigs=[simplify(weig/the_pivot) for weig in weigs]
#     for k in range(len(weigs)):
#         if weigs[k]==the_pivot:
#             weigs[k]==True
#         elif weigs[k]== ~the_pivot:
#             weigs[k]==False
#         else:
#             weigs[k]=simplify(weig[k]/the_pivot)
        
    succ_nodes=[succ.node for succ in the_successors]
    node=Find_Or_Add_Unique_table(x,weigs,succ_nodes)
    res=TDD(node)
    res.weight=the_pivot
    return res
...
def find_computed_table(item):
    ...
    if item[0]=='s':
        ...
        the_key=('+',item[1].weight,item[1].node,item[2].weight,item[2].node)
        add_find_time+=1
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            tdd = TDD(res[1])
            ...
            add_hit_time+=1
            return tdd
        the_key=('+',item[2].weight,item[2].node,item[1].weight,item[1].node)
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            tdd = TDD(res[1])
            tdd.weight = res[0]
        ...
        the_key=('*',item[1].weight,item[1].node,item[2].weight,item[2].node,item[3][0],item[3][1],item[4])
        cont_find_time+=1
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            tdd = TDD(res[1])
            ...
            cont_hit_time+=1            
            return tdd
        the_key=('*',item[2].weight,item[2].node,item[1].weight,item[1].node,item[3][1],item[3][0],item[4])
        if computed_table.__contains__(the_key):
            res = computed_table[the_key]
            tdd = TDD(res[1])
            tdd.weight = res[0]
...
def add(tdd1,tdd2):
    ...
    if tdd1.node==tdd2.node:
        weig=simplify(tdd1.weight+tdd2.weight)
        if weig==0:
