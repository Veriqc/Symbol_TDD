...
def mul(bdd1, bdd2):
    ...
    for term1 in bdd1.data:
        for term2 in bdd2.data:
            ...
            t = tuple(
                np.round(float(term1[i] + term2[i]), int(-np.log10(epi)))
                for i in range(len(term1))
            )

            new_r = np.round(
                bdd1.data[term1][0] * bdd2.data[term2][0], int(-np.log10(epi))
            )
            new_theta = np.round(
                bdd1.data[term1][1] + bdd2.data[term2][1], int(-np.log10(epi))
            )
