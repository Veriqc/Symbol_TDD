...
class BDD:
    ...
    def show(self, real_label=True):
        ...
        dot.edge(
            ...
            str(self.node.idx),
            color="blue",
            label=str(complex(round(self.weight.real, 2), round(self.weight.imag, 2))),
        )
        dot.format = "png"
...
def layout(node, dot=Digraph(), succ=[]):
    ...
    for k in range(node.succ_num):
        if node.successor[k]:
            label1 = str(
                complex(
                    round(node.out_weight[k].real, 2), round(node.out_weight[k].imag, 2)
                )
            )
...
def normalize_2_fun(tdd1, tdd2):
    ...
    [a1, b1, c1] = normalize_2_fun(
        Slicing2(tdd1, the_key, 0), Slicing2(tdd2, the_key, 0)
    )
    [a2, b2, c2] = normalize_2_fun(
        Slicing2(tdd1, the_key, 1), Slicing2(tdd2, the_key, 1)
    )
...
def get_expr(node):
    ...
    if l == r:
        if abs(node.out_weight[0] - node.out_weight[1]) < epi:
            ...
            res = (
                node.out_weight[0] * symbols(xn) + node.out_weight[1] * symbols(x)
            ) * l
    else:
