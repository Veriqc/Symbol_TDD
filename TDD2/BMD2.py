...
#         the_successors.append(res)
    left=mul(Slicing(tdd1,the_key,0),Slicing(tdd2,the_key,0))
    right1=mul(Slicing(tdd1,the_key,0),Slicing(tdd2,the_key,1))
    right2=mul(Slicing(tdd1,the_key,1),Slicing(tdd2,the_key,0))
    right3=mul(Slicing(tdd1,the_key,1),Slicing(tdd2,the_key,1))
    right=add(right1,right2)
    right=add(right,right3)
    tdd=normalize(the_key,[left,right])
...
def get_all_nodes(A):
    node_set=set()
    for tdd in A:
        node_set=get_node_set(tdd.node,node_set)
    return len(node_set)
def normalize_2_fun(tdd1,tdd2):
    #tdd2/tdd1
     
    if tdd1.weight==0 and tdd2.weight==0:
        return [tdd1.self_copy(),tdd1.self_copy(),tdd2.self_copy()]
    if tdd1.weight==0:
#         print('a')
...
#         print('d')
    ...
    f0=Slicing2(tdd1,the_key,0)
    f1=add(Slicing2(tdd1,the_key,0),Slicing2(tdd1,the_key,1))
        
    g0=Slicing2(tdd2,the_key,0)
    g1=add(Slicing2(tdd2,the_key,0),Slicing2(tdd2,the_key,1))        
    ...
    if get_all_nodes([a,b,c])<=get_all_nodes([tdd1,tdd2])+1:
...
#         res=node.out_weight[0]*symbols(xn)*r+node.out_weight[1]*symbols(x)*l
    res=node.out_weight[0]*l+symbols(x)*node.out_weight[1]*r
    node.expr=res
    return res